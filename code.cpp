    ...
    while (true) 
    {
        ...
        if (!img.empty()) {
            count_num=count_num+1;
                ...
                //  foreground_BW, element);
            darts_roi=img1(Rect((img1.cols/2)-20,(img1.rows/2)-20,140,140));
	    	rectangle(img1, Rect((img1.cols/2)-20,(img1.rows/2)-20,140,140), Scalar(255, 255, 0), 2);	//在原图像上画出矩形
            ...
            time = ((double)getTickCount() - time) / getTickFrequency();
            int fps = 1 / time;
            cv::putText(img, fmt::format("fps={}", fps), {10, 25}, cv::FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(0, 0, 0));
            cout<<"fps:"<<fps<<endl;
            // frame_0=mid_filer.clone();
            cv::imshow("0", img);
            if(cv::waitKey(1) == 'q') {
